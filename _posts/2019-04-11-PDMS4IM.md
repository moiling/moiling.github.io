...
$$g_{1}(x) = \parallel C_{x} - C_{z} \parallel_{2}$$

$$g_{2}(x) = \parallel S_{x} - S_{z} \parallel_{2}$$

$$g_{3}(x) = \parallel T_{x} - T_{z} \parallel_{2}$$
...
$$O_{z}(x_{F},x_{B}) = O_z^{(c)}(x_{F},x_{B})\times O_z^{(s)}(x_{F},x_{B})$$

$$O_z^{(c)}(x_{F},x_{B})=\exp(-\parallel C_{z}-(\hat{\alpha_{z}}C_{x_{F}}+(1-\hat{\alpha_{z}})C_{x_{B}}))\parallel_{2})$$

$$O_z^{(s)}(x_{F},x_{B})=\exp(\frac{-\parallel S_{z}-S_{x_{F}}\parallel_{2}}{\frac{1}{|\Omega_{F}|}\sum_{x_{i}\in\Omega_{F}}\parallel S_{z}-S_{x_{i}}\parallel_{2}})\times\exp(\frac{-\parallel S_{z}-S_{x_{B}}\parallel_{2}}{\frac{1}{|\Omega_{B}|}\sum_{x_{i}\in\Omega_{B}}\parallel S_{z}-S_{x_{i}}\parallel_{2}})$$
